...
if ($action == 'create') {
    ...
    // Jika semuanya baik, coba unggah file
        ...
        if (move_uploaded_file($_FILES['image']['tmp_name'], $target_file)) {
            ...
            $sql = "INSERT INTO laptops (name, image, description, price) VALUES ('$name', '$target_file', '$description', '$price')";
            if ($conn->query($sql) === TRUE) {
                echo json_encode(["status" => "success", "message" => "laptops created successfully"]);
            } else {
...
if ($action == 'read') {
    $sql = "SELECT laptops.* FROM laptops";
...
if ($action == 'update') {
    ...
    $updateQuery = "UPDATE laptops SET name='$name', description='$description', price='$price'";
    ...
    if ($conn->query($updateQuery) === TRUE) {
        echo json_encode(["status" => "success", "message" => "laptops updated successfully"]);
    } else {
        echo json_encode(["status" => "fail", "message" => "Error: " . $conn->error]);
    }
}
...
if ($action == 'delete') {
    ...
    if ($conn->query($sql) === TRUE) {
        echo json_encode(["status" => "success", "message" => "laptops deleted successfully"]);
    } else {
        echo json_encode(["status" => "fail", "message" => "Error: " . $conn->error]);
    }
}
